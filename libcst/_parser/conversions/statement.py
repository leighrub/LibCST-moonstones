...
from libcst._nodes.statement_spanish import Continúa, Rompe
...
@with_production(
    "simple_stmt_partial", "small_stmt (';' small_stmt)* [';'] NEWLINE"
)
def convert_simple_stmt_partial(
...
def convert_simple_stmt_suite(
  ...
  return SimpleStatementSuite(
      partial.body,
      leading_whitespace=parse_simple_whitespace(
          config, partial.whitespace_before
      ),
      trailing_whitespace=partial.trailing_whitespace,
  )
...
@with_production(
    "small_stmt",
    (
        "expr_stmt | del_stmt | pass_stmt | break_stmt | continue_stmt |"
        " return_stmt"
        + "| raise_stmt | yield_stmt | import_stmt | global_stmt |"
        " nonlocal_stmt"
        + "| assert_stmt"
        # Spanish stmts
        + "| rompe_stmt | continúa_stmt | devuelve_stmt | aumenta_stmt"
    ),
)
...
@with_production("continúa_stmt", "'continúa'")
def convert_continúa_stmt(config: ParserConfig, children: Sequence[Any]) -> Any:
  (name,) = children
  return WithLeadingWhitespace(Continúa(), name.whitespace_before)
...
@with_production("rompe_stmt", "'rompe'")
def convert_rompe_stmt(config: ParserConfig, children: Sequence[Any]) -> Any:
  (name,) = children
  return WithLeadingWhitespace(Rompe(), name.whitespace_before)


@with_production("return_stmt", "'return' [testlist]", version="<=3.7")
@with_production(
    "return_stmt", "'return' [testlist_star_expr]", version=">=3.8"
)
...
@with_production("devuelve_stmt", "'devuelve' [testlist]", version="<=3.7")
@with_production(
    "devuelve_stmt", "'devuelve' [testlist_star_expr]", version=">=3.8"
)
def convert_devuelve_stmt(config: ParserConfig, children: Sequence[Any]) -> Any:
  if len(children) == 1:
    (keyword,) = children
    return WithLeadingWhitespace(
        Return(whitespace_after_devuelve=SimpleWhitespace("")),
        keyword.whitespace_before,
    )
  else:
    (keyword, testlist) = children
    return WithLeadingWhitespace(
        Return(
            value=testlist.value,
            whitespace_after_devuelve=parse_simple_whitespace(
                config, keyword.whitespace_after
            ),
        ),
        keyword.whitespace_before,
...
@with_production(
    "import_from",
    "'from' import_relative 'import' ('*' | '(' import_as_names ')' |"
    " import_as_names)",
)
...
def convert_raise_stmt(config: ParserConfig, children: Sequence[Any]) -> Any:
  ...
  return WithLeadingWhitespace(
      Raise(
          whitespace_after_raise=whitespace_after_raise, exc=exc, cause=cause
      ),
      raise_token.whitespace_before,
  )


@with_production("aumenta_stmt", "'aumenta' [test ['from' test]]")
def convert_aumenta_stmt(config: ParserConfig, children: Sequence[Any]) -> Any:
  if len(children) == 1:
    (aumenta_token,) = children
    whitespace_after_aumenta = MaybeSentinel.DEFAULT
    exc = None
    cause = None
  elif len(children) == 2:
    (aumenta_token, test) = children
    whitespace_after_aumenta = parse_simple_whitespace(
        config, test.whitespace_before
    )
    exc = test.value
    cause = None
  elif len(children) == 4:
    (aumenta_token, test, from_token, source) = children
    whitespace_after_aumenta = parse_simple_whitespace(
        config, test.whitespace_before
    )
    exc = test.value
  ...
  return WithLeadingWhitespace(
      Raise(
          whitespace_after_aumenta=whitespace_after_aumenta, exc=exc, cause=cause
      ),
      aumenta_token.whitespace_before,
  )
...
@with_production(
    "if_stmt",
    "'if' test ':' suite [if_stmt_elif|if_stmt_else]",
    version="<=3.7",
)
@with_production(
    "if_stmt",
...
def convert_if_stmt(config: ParserConfig, children: Sequence[Any]) -> Any:
  ...
  return If(
      ...
      whitespace_before_test=parse_simple_whitespace(
          config, if_tok.whitespace_after
      ),
      test=test.value,
...
@with_production(
    "if_stmt_elif",
    "'elif' test ':' suite [if_stmt_elif|if_stmt_else]",
    version="<=3.7",
)
@with_production(
    "if_stmt_elif",
...
@with_production(
    "try_stmt",
    "('try' ':' suite ((except_clause ':' suite)+ ['else' ':' suite] ['finally'"
    " ':' suite] | 'finally' ':' suite))",
)
...
@with_production(
    "funcdef", "'def' NAME parameters [funcdef_annotation] ':' suite"
)
...
def convert_funcdef_annotation(
    config: ParserConfig, children: Sequence[Any]
) -> Any:
...
def convert_classdef(config: ParserConfig, children: Sequence[Any]) -> Any:
  ...
  whitespace_after_class = parse_simple_whitespace(
      config, classdef.whitespace_after
  )
...
def convert_decorator(config: ParserConfig, children: Sequence[Any]) -> Any:
  ...
  return Decorator(
      ...
      whitespace_after_at=parse_simple_whitespace(
          config, atsign.whitespace_after
      ),
...
@with_production(
    "asyncable_stmt", "funcdef | with_stmt | for_stmt", version="<3.5"
)
def convert_asyncable_stmt(
    config: ParserConfig, children: Sequence[Any]
) -> Any:
